...
const SETUPS = ["Order Block", "FVG", "BOS/CHOCH", "Liquidity Sweep", "Support/Résistance", "Trend Follow", "Autre"];
...
export default function TradingJournal() {
  ...
  return (
    <div style={{
      ...
      fontFamily: "'Inter', -apple-system, sans-serif", padding: "0 0 60px"
    }}>
      {/* Header */}
      <div style={{
      ...
      {/* Tabs */}
      <div style={{ display: "flex", gap: 0, borderBottom: "1px solid #111", padding: "0 20px" }}>
              ...
              {filtered.map(trade => (
                <div key={trade.id} style={{
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    ...
                    <div style={{
                      color: pnlColor(trade.pnl), fontWeight: 700, fontFamily: "monospace",
                      fontSize: 15
                    }}>
                  ...
                  {trade.setupType && (
                    <div style={{ marginTop: 8 }}>
                      <span style={{ background: "#111", color: "#888", borderRadius: 4, padding: "2px 7px", fontSize: 10 }}>
                        {trade.setupType}
                      </span>
                    </div>
                  )}
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 10 }}>
                    <div style={{ color: "#2a2a2a", fontSize: 10 }}>
                      ...
                      {trade.emotion && trade.emotion !== "Neutre" && <span> • {trade.emotion}</span>}
                    </div>
        ...
        <div style={{ padding: "16px" }}>
          {trades.length === 0 ? (
            ...
            <>
              ...
              {/* Par session */}
              <div style={{ marginBottom: 20 }}>
                <div style={{ color: "#333", fontSize: 10, textTransform: "uppercase", letterSpacing: 1, marginBottom: 10 }}>Performance par session</div>
                  ...
                  <div key={s.name} style={{
                    ...
                    <div style={{ textAlign: "right" }}>
                      <div style={{ color: "#c8a84b", fontWeight: 700, fontFamily: "monospace" }}>
                        {s.count ? Math.round((s.wins / s.count) * 100) : 0}%
                      </div>
                      <div style={{ color: "#333", fontSize: 10 }}>win rate</div>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {(() => {
                const emotionMap = {};
                trades.forEach(t => {
                  if (!emotionMap[t.emotion]) emotionMap[t.emotion] = { total: 0, wins: 0 };
                  emotionMap[t.emotion].total++;
                  if (t.result === "WIN") emotionMap[t.emotion].wins++;
                });
                const entries = Object.entries(emotionMap).filter(([, v]) => v.total > 0);
                if (!entries.length) return null;
                return (
                  <div>
                    <div style={{ color: "#333", fontSize: 10, textTransform: "uppercase", letterSpacing: 1, marginBottom: 10 }}>Performance par état émotionnel</div>
      ...
      <Modal show={showForm} onClose={() => { setShowForm(false); setEditId(null); }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 12px" }}>
          ...
          <Input label="P&L ($)" value={form.pnl} onChange={f("pnl")} placeholder="ex: +120 ou -80" />
        ...
        <Input label="Type de setup" value={form.setupType} onChange={f("setupType")} options={["", ...SETUPS]} />
        <div style={{ marginBottom: 12 }}>
